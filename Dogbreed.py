...
main = tkinter.Tk()
main.title("Dog Bread Classification system")
...
labels = ['afghan_hound', 'african_hunting_dog', 'airedale', 'basenji']
...
def runCNN():
    ...
    text.insert(END,"CNN Dog breed Accuracy  : "+str(a)+"\n")
    text.insert(END,"CNN Dog breed Precision : "+str(p)+"\n")
    text.insert(END,"CNN Dog  breed Recall    : "+str(r)+"\n")
    text.insert(END,"CNN Dog breed FSCORE    : "+str(f)+"\n\n")
...
def graph():
    ...
    plt.title('CNN Dog breed Training Accuracy & Loss Graph')
    plt.show()
...
def predict():
    ...
    cv2.putText(img, 'Dog breed Predicted as : '+labels[predict], (10, 25),  cv2.FONT_HERSHEY_SIMPLEX,0.7, (0, 0, 255), 2)
    cv2.imshow('Dog breed Predicted as  : '+labels[predict], img)
    cv2.waitKey(0)
...
font = ('times', 15, 'bold')
title = Label(main, text='Dog Breed Classification system', justify=LEFT)
...
font1 = ('times', 12, 'bold')
uploadButton = Button(main, text="Upload Dog Breed Dataset", command=uploadDataset)
...
predictButton = Button(main, text="Predict Dog Breed", command=predict)
